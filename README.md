...
0. **QA pass on the live app surfaced real bugs — all fixed, documented honestly below.**
   ...
   - **Same class of gap found in `prescriptionOptions`.** The "Prescription only" badge list wasn't being filtered by the patient's safety profile at all — a pregnant patient could still see "Hydroquinone 4%" offered as a badge. Fixed with a dedicated filter applied before the badge list is returned.
   - **Retinaldehyde was in the live routines without ever being an approved ingredient in `MASTER_ALGORITHM.md`.** Corrected — Retinaldehyde and Retinol are now both properly documented in Section 4 with concentration ranges and cited evidence (Creidi et al. 1998, Saurat et al. 1994, Kafi et al. 2007), positioned by potency, Bakuchiol beneath both as the gentler non-retinoid alternative.
   - **Missing AM moisturizer** in the Acne Pregnancy-Safe routine — added.
   - **Upsells cleaned up across the rule set.** Several rules listed something as an "upsell" that was already a mandatory step in that same rule's own routine (SPF shown as optional add-on when it's already required AM; a retinoid/Tranexamic Acid product duplicating the routine's own active). Every upsell is now something genuinely additional, not a rebranded mandatory step.
   - **Menopause question (1.4) now age-gated** — hidden for patients under 36, since asking an 18-year-old about menopause doesn't make clinical sense. Contraception/HRT (1.5) and cycle-pattern (1.6) stay visible regardless of age, since both remain relevant to younger patients.


0. **Substantial clinical/legal/UX build pass.** Several things fixed and added together:
   - **Pre-checked Yes/No bug fixed.** Every yes/no question in the assessment (8 places across `SafetyStep`, `SkinTypeStep`, `SeverityStep`) previously defaulted to `false`, which the `YesNo` component rendered identically to an explicit "No" — a fresh, untouched assessment showed "No" pre-selected on questions nobody had answered. Fixed by making these fields `boolean | null` end to end (form state → API → schema); `null` now means "unanswered" and renders nothing selected, exactly as it should.
   - **Two new conditional follow-up questions.** "Does your skin change around your cycle?" and "recent texture change?" now branch: a Yes to the cycle question asks *how* (oilier/breakouts, more sensitive, drier) — "oilier/breakouts" nudges the matching engine toward Azelaic Acid/Niacinamide-anchored rules when Acne is also selected, per the Personalization Rules table in `MASTER_ALGORITHM.md`. A Yes to the texture question asks *what* changed — "more sensitive" now feeds `sensitiveOverlay` directly, on equal footing with the existing 2.4/2.5 reactivity questions, since a newly-reactive patient should get the gentler tier regardless of which question caught it.
   - **Benzoyl Peroxide spot-treatment tip** added to the three standard (non-pregnancy) acne rules — framed explicitly as a supplementary tip for an isolated new breakout, not a routine step, since whole-area treatment remains the correct default (acne starts before it's visible; spot-treating only what's already visible leaves the rest of the follicular process untreated).
   - **Real prescription-tier badge system, replacing prose-only escalation notes.** Tretinoin, Tazarotene, and above-2%/above-1% Hydroquinone/Kojic Acid previously only ever appeared as free text inside `escalationNote` ("refer to a dermatologist..."). They're now a structured `prescriptionOptions` field on each rule, shown to **every** clinic with a "Prescription only" badge — informational only. A `Dispense` action only ever appears when **both** the clinic is a verified pharmacy **and** the specific logged-in person has been tagged `PHARMACIST` by their own Clinic Admin — enforced server-side in `/api/sales`, not just hidden in the UI. In Nigeria, none of these are controlled drugs, so a pharmacist's own clinical discretion is sufficient to dispense — no prescription-recording step required. (Worth revisiting if/when this expands beyond Nigeria — other jurisdictions may draw this line differently.)
   - **New `StaffType` (Pharmacist / Support Staff), set only by Clinic Admin**, at `/dashboard/staff` — applies to Staff and Clinic Admin accounts alike, since a clinic's owner is often the pharmacist themselves. Nobody is tagged Pharmacist by default.
   - **Named real ingredients in 14 previously-generic moisturizer steps** ("Moisturizer" → "Moisturizer (Hyaluronic Acid + Ceramides + Niacinamide)", etc.), and enriched several thin upsell lists — your docs cover 124 distinct ingredients, only 59 were actually appearing anywhere in the 20 rules; this pass draws on more of what was already vetted but unused, without adding anything new.
   - **`prisma/seed.ts` and `scripts/generate-seed-sql.ts` both updated** to actually pass the new `prescriptionOptions` field through — confirmed by running the SQL generator directly; it now emits it correctly.
   - **Deliberately not done in this pass:** the AI-generated assessment summary (needs its own `ANTHROPIC_API_KEY`-backed build, kept separate on purpose) and Clascoterone (still unconfirmed whether it's available in Nigeria at all).
