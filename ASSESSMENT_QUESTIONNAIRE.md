...
| 1.4 | *(shown only if 1.2 = Female or "prefer not to say", 1.3 ≠ Pregnant/Breastfeeding, **[NEW]** and age range is 36+)* Have you gone through menopause? | Not yet / Perimenopausal (periods becoming irregular) / Yes, menopausal / Not applicable | Direct question is the source of truth — **not** inferred from age alone, since age bands overlap in real life. **[NEW]** Age is used only to decide whether to *ask* at all (below 36, it's skipped entirely as clinically implausible) — never to infer the *answer* for someone in the plausible range | `Assessment.hormonalStage` |
